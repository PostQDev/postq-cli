...
|---|---|---|
| `scan_url` | Live TLS handshake; uploads by default and returns a dashboard URL; `upload: false` runs offline | Yes by default; no offline |
...
`scan_url` uploads to the dashboard by default and `scan_code` is always local.
Call `scan_url` with `upload: false` when you explicitly need an offline scan. The CLI
...
- `scan_url` uploads by default. The caller can explicitly supply `upload: false`
  for an offline scan. Uploaded scans use the API key resolved by the pinned PostQ CLI and belong to
...
| URL scan succeeds but does not appear in the dashboard | Confirm authentication with `postq auth whoami`, verify the response contains a `portalUrl`, and confirm the dashboard is showing the organization that issued the active API key. Also ensure the call did not set `upload: false`. |
